...
class QualitySpider(object):
    def __init__(self):
        ...
        self.session_production = scoped_session(sessionmaker(bind=self.engine_production))   #类似于pymsql里面的游标
    ...
    def upadate_email_data(self):
        data = self.session_test.query(SpiderEmail).filter().first()   ## 使用first方法获取结果集中的第一条数据，all()则为获取全部
    ...
    def runsanp(self,scraper):
        ...
        for item in list:
            try:
                ...
                # TODO 考虑为spider_article_list表加上tag字段进行查询筛选，减少spider_article_list查询结果list的数量,
    ...
    def runlist(self, scraper):
        ...
        while True:
            ...
            items = find(driver, scraper.byItem)   #大标题列表
            dates = find(driver, scraper.byItemDate) #大时间列表
            ...
            for i in items:
                _dx = _dx+1
                try:
                    title = items[idx].get_attribute('title').replace('\n', '').replace('<P>', '').replace('</P>', '').replace('\u2002', '”')  #获取具体的标题   获取第一个元素，get_attribute('title')获取得到的是value，得到每个具体的标题
                    if 'cat:' in title:
                ...
                try:
                    addr = items[idx].get_attribute("href").split(';')[0]   # 获取具体的网站   获取每个标题对应的网站split(';')[0]得到后面具体的值
                except Exception as err:
                ...
                if scraper.ID == 1367371348984475648:
                    ...
                    addr = f'http://yn315.org.cn/html/details.html?articleId={addr_id}&channelId={channel_id}&rules=比较试验'
                ...
                print(addr)
                if scraper.byItemTitle:  #判断是否有这个，因为上面的语句没有办法获取到标题，所以一些网站需要特殊处理
    ...
    def run_take(self, take):
        for scraper in take:
            self.logger.info("{}\t{}\t{}".format(scraper.name, scraper.source_url, scraper.areaCode))     #记录日志
    ...
    def run(self):
        ...
        try:
            ...
            for _id in [
                ...
                first_ret = self.session_production.query(QualityScraper).filter(QualityScraper.valid == 1,     #first_ret返回表里面要查询的各个字段，filter()里面维查询的语句
    ...
    def __del__(self):
        self.driver.quit()  #driver.quit()退出驱动关闭所有窗口
